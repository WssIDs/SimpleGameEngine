...
void Material::Load()
{
	...
	{
		Graphics::GetGraphics().pDevice3D->CreateVertexShader(VertexShaderBuffer->GetBufferPointer(), VertexShaderBuffer->GetBufferSize(), nullptr, &VertexShader);
		Graphics::GetGraphics().pDevice3D->CreatePixelShader(PixelShaderBuffer->GetBufferPointer(), PixelShaderBuffer->GetBufferSize(), nullptr, &PixelShader);

		if (VertexShader.Get() != nullptr || PixelShader.Get() != nullptr)
		{
			Graphics::GetGraphics().pDeviceContext3D->VSSetShader(VertexShader.Get(), nullptr, 0);
			Graphics::GetGraphics().pDeviceContext3D->PSSetShader(PixelShader.Get(), nullptr, 0);
